...
common::bytecodes::ApicaTypeBytecode ValueU64::getKind() const {
    return common::bytecodes::ApicaTypeBytecode::U64;
}

std::optional<Value*> ValueU64::add(const Value *other) const {
    switch (other->getKind()) {
        case common::bytecodes::ApicaTypeBytecode::I8: {
            const ValueI8 *i8 = static_cast<const ValueI8*>(other);
            return new ValueI64(this->value.value() + i8->getValue().value());
        }

        case common::bytecodes::ApicaTypeBytecode::I16: {
            const ValueI16 *i16 = static_cast<const ValueI16*>(other);
            return new ValueI64(this->value.value() + i16->getValue().value());
        }

        case common::bytecodes::ApicaTypeBytecode::I32: {
            const ValueI32 *i32 = static_cast<const ValueI32*>(other);
            return new ValueI64(this->value.value() + i32->getValue().value());
        }
        ...
        case common::bytecodes::ApicaTypeBytecode::Bool: {
            ...
            return new ValueU64(this->value.value() + boolean->getValue().value());
        }

        case common::bytecodes::ApicaTypeBytecode::Char: {
            const ValueChar *character = static_cast<const ValueChar*>(other);
            return new ValueU64(this->value.value() + character->getValue().value());
        }
...
std::optional<Value*> ValueU64::increment() {
    return new ValueU64(this->value.value()++);
}

std::optional<Value*> ValueU64::leftIncrement() {
    return new ValueU64(++this->value.value());
}

std::optional<Value*> ValueU64::subtract(const Value *other) const {
    switch (other->getKind()) {
        case common::bytecodes::ApicaTypeBytecode::I8: {
            const ValueI8 *i8 = static_cast<const ValueI8*>(other);
            return new ValueI64(this->value.value() - i8->getValue().value());
...
std::optional<Value*> ValueU64::decrement() {
    return new ValueU64(this->value.value()--);
}

std::optional<Value*> ValueU64::leftDecrement() {
    return new ValueU64(--this->value.value());
}

std::optional<Value*> ValueU64::times(const Value *other) const {
    switch (other->getKind()) {
        case common::bytecodes::ApicaTypeBytecode::I8: {
            const ValueI8 *i8 = static_cast<const ValueI8*>(other);
            return new ValueI64(this->value.value() * i8->getValue().value());
        ...
        case common::bytecodes::ApicaTypeBytecode::Bool: {
            const ValueBool *boolean = static_cast<const ValueBool*>(other);
            return new ValueU64(boolean->getValue().value() ? this->value.value() : 0);
        }
        ...
        case common::bytecodes::ApicaTypeBytecode::String: {
            ...
            for (uint64_t i = 0; i < this->value.value(); i++)
...
std::optional<Value*> ValueU64::unaryNot() const {
    return new ValueBool(this->value.has_value() ? !this->value.value() : true);
}

std::optional<Value*> ValueU64::bitwiseNot() const {
    return new ValueU64(~this->value.value());
}

std::optional<Value*> ValueU64::lessThan(const Value *other) const {
    switch (other->getKind()) {
        case common::bytecodes::ApicaTypeBytecode::I8: {
            const ValueI8 *i8 = static_cast<const ValueI8*>(other);
            return new ValueBool(this->value.value() < static_cast<uint8_t>(i8->getValue().value()));
        }

        case common::bytecodes::ApicaTypeBytecode::I16: {
            const ValueI16 *i16 = static_cast<const ValueI16*>(other);
            return new ValueBool(this->value.value() < static_cast<uint16_t>(i16->getValue().value()));
        }

        case common::bytecodes::ApicaTypeBytecode::I32: {
            const ValueI32 *i32 = static_cast<const ValueI32*>(other);
            return new ValueBool(this->value.value() < static_cast<uint32_t>(i32->getValue().value()));
        }

        case common::bytecodes::ApicaTypeBytecode::I64: {
            const ValueI64 *i64 = static_cast<const ValueI64*>(other);
            return new ValueBool(this->value.value() < static_cast<uint64_t>(i64->getValue().value()));
        }

        case common::bytecodes::ApicaTypeBytecode::U8: {
            const ValueU8 *u8 = static_cast<const ValueU8*>(other);
            return new ValueBool(this->value.value() < u8->getValue().value());
        }

        case common::bytecodes::ApicaTypeBytecode::U16: {
            const ValueU16 *u16 = static_cast<const ValueU16*>(other);
            return new ValueBool(this->value.value() < u16->getValue().value());
        }

        case common::bytecodes::ApicaTypeBytecode::U32: {
            const ValueU32 *u32 = static_cast<const ValueU32*>(other);
            return new ValueBool(this->value.value() < u32->getValue().value());
        }

        case common::bytecodes::ApicaTypeBytecode::U64: {
            const ValueU64 *u64 = static_cast<const ValueU64*>(other);
            return new ValueBool(this->value.value() < u64->getValue().value());
        }
        ...
        case common::bytecodes::ApicaTypeBytecode::Char: {
            const ValueChar *character = static_cast<const ValueChar*>(other);
            return new ValueBool(this->value.value() < character->getValue().value());
        }
...
std::optional<Value*> ValueU64::lessOrEquals(const Value *other) const {
    switch (other->getKind()) {
        case common::bytecodes::ApicaTypeBytecode::I8: {
            const ValueI8 *i8 = static_cast<const ValueI8*>(other);
            return new ValueBool(this->value.value() <= static_cast<uint8_t>(i8->getValue().value()));
...
std::optional<Value*> ValueU64::greaterThan(const Value *other) const {
    switch (other->getKind()) {
        case common::bytecodes::ApicaTypeBytecode::I8: {
            const ValueI8 *i8 = static_cast<const ValueI8*>(other);
            return new ValueBool(this->value.value() > static_cast<uint8_t>(i8->getValue().value()));
...
std::optional<Value*> ValueU64::greaterOrEquals(const Value *other) const {
    switch (other->getKind()) {
        case common::bytecodes::ApicaTypeBytecode::I8: {
            const ValueI8 *i8 = static_cast<const ValueI8*>(other);
            return new ValueBool(this->value.value() >= static_cast<uint8_t>(i8->getValue().value()));
...
std::optional<Value*> ValueU64::convert(common::bytecodes::ApicaTypeBytecode to) const {
    if (this->value) {
        switch (to) {
            case common::bytecodes::ApicaTypeBytecode::String:
            ...
            case common::bytecodes::ApicaTypeBytecode::Type:
                return new ValueType(common::bytecodes::ApicaTypeBytecode::U64);
...
std::optional<Value*> ValueU64::autoConvert(common::bytecodes::ApicaTypeBytecode to) const {
    if (this->value) {
        switch (to) {
