    ...
    pub fn convert(&self, to: ApicaTypeBytecode) -> Option<Value> {
        if let Some(value) = &self.value {
            match to {
                ...
                ApicaTypeBytecode::Type => Some(Value::Type(ValueType::init_with(ApicaTypeBytecode::U16, None))),
    ...
    pub fn auto_convert(&self, to: ApicaTypeBytecode) -> Option<Value> {
        if let Some(value) = &self.value {
            match to {
                ...
                ApicaTypeBytecode::U16 => Some(Value::U16(ValueU16::init_with(*value))),
...
mod tests {
    ...
    #[test]
    fn test_empty() {
        let mut u16 = ValueU16::init_empty();
        assert!(u16.is_null());

        u16 = ValueU16::init_with(12);
        assert!(!u16.is_null());
    }

    #[test]
    fn test_type_repr() {
        let u16 = ValueU16::init_empty();
        assert_eq!("u16", u16.get_type_representation());
    }

    #[test]
    fn test_get_value() {
        let mut u16 = ValueU16::init_empty();
        assert!(u16.get_value().is_none());

        u16 = ValueU16::init_with(12);
        assert_eq!(12, u16.get_value().unwrap());
    }

    #[test]
    fn test_add() {
        let u16 = ValueU16::init_with(12);

        let i16 = Value::I16(ValueI16::init_with(-12));
        let mut result = u16.add(&i16);
        ...
        result = u16.add(&string);
        assert!(result.is_none());
    }

    #[test]
    fn test_increment() {
        let mut u16 = ValueU16::init_with(12);
        let result = u16.increment();
        if let Value::U16(v) = &result.unwrap() {
            assert_eq!(12, v.get_value().unwrap());
        } else { panic!(); }

        assert_eq!(13, u16.get_value().unwrap());
    }

    #[test]
    fn test_decrement() {
        let mut u16 = ValueU16::init_with(12);
        let result = u16.decrement();
        if let Value::U16(v) = &result.unwrap() {
            assert_eq!(12, v.get_value().unwrap());
        }

        assert_eq!(11, u16.get_value().unwrap());
    }

    #[test]
    fn test_not() {
        let mut u16 = ValueU16::init_empty();
        let mut result = u16.not();
        if let Value::Bool(bool_result) = &result {
            assert_eq!(true, bool_result.get_value().unwrap());
        } else { panic!(); }

        u16 = ValueU16::init_with(0);
        result = u16.not();
        if let Value::Bool(bool_result) = &result {
            assert_eq!(true, bool_result.get_value().unwrap());
        } else { panic!(); }

        u16 = ValueU16::init_with(1);
        result = u16.not();
        if let Value::Bool(bool_result) = &result {
