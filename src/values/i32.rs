...
impl ValueI32 {
    ...
    pub fn add(&self, other: &Value) -> Option<Value> {
        match other {
            Value::I8(i8) => Some(Value::I32(ValueI32::init_with(self.value.unwrap() + i8.get_value().unwrap() as i32))),
            Value::I16(i16) => Some(Value::I32(ValueI32::init_with(self.value.unwrap() + i16.get_value().unwrap() as i32))),
            Value::I32(i32) => Some(Value::I32(ValueI32::init_with(self.value.unwrap() + i32.get_value().unwrap()))),
            Value::I64(i64) => Some(Value::I64(ValueI64::init_with(self.value.unwrap() as i64 + i64.get_value().unwrap()))),
            Value::U8(u8) => Some(Value::I32(ValueI32::init_with(self.value.unwrap() + u8.get_value().unwrap() as i32))),
            Value::U16(u16) => Some(Value::I32(ValueI32::init_with(self.value.unwrap() + u16.get_value().unwrap() as i32))),
            Value::U32(u32) => Some(Value::I32(ValueI32::init_with(self.value.unwrap() + u32.get_value().unwrap() as i32))),
            ...
            Value::Bool(bool) => Some(Value::I32(ValueI32::init_with(self.value.unwrap() + if bool.get_value().unwrap() { 1 } else { 0 }))),

            Value::Char(char) => Some(Value::I32(ValueI32::init_with(self.value.unwrap() + char.get_value().unwrap() as i32))),
    ...
    pub fn increment(&mut self) -> Option<Value> {
        if let Some(value) = self.value.as_mut() {
            let old_value = Value::I32(ValueI32::init_with(*value));
            *value += 1;
            Some(old_value)
        } else {
            unreachable!()
    ...
    pub fn convert(&self, to: ApicaTypeBytecode) -> Option<Value> {
        if let Some(value) = &self.value {
            match to {
                ...
                ApicaTypeBytecode::Type => Some(Value::Type(ValueType::init_with(ApicaTypeBytecode::I32, None))),
    ...
    pub fn auto_convert(&self, to: ApicaTypeBytecode) -> Option<Value> {
        if let Some(value) = &self.value {
            match to {
                ApicaTypeBytecode::Any => Some(Value::I32(ValueI32::init_with(*value))),
                ...
                ApicaTypeBytecode::I32 => Some(Value::I32(ValueI32::init_with(*value))),
            ...
            match to {
                ApicaTypeBytecode::Any => Some(Value::I32(ValueI32::init_empty())),
...
mod tests {
    ...
    #[test]
    fn test_empty() {
        let mut i32 = ValueI32::init_empty();
        assert!(i32.is_null());

        i32 = ValueI32::init_with(12);
        assert!(!i32.is_null());
    }

    #[test]
    fn test_type_repr() {
        let i32 = ValueI32::init_empty();
        assert_eq!("i32", i32.get_type_representation());
    }

    #[test]
    fn test_get_value() {
        let mut i32 = ValueI32::init_empty();
        assert!(i32.get_value().is_none());

        i32 = ValueI32::init_with(12);
        assert_eq!(12, i32.get_value().unwrap());
    }

    #[test]
    fn test_add() {
        let i32 = ValueI32::init_with(12);

        let i16 = Value::I16(ValueI16::init_with(-12));
        let mut result = i32.add(&i16);
        ...
        result = i32.add(&string);
        assert!(result.is_none());
    }

    #[test]
    fn test_increment() {
        let mut i32 = ValueI32::init_with(12);
        let result = i32.increment();
        if let Value::I32(v) = &result.unwrap() {
            assert_eq!(12, v.get_value().unwrap());
        } else { panic!(); }

        assert_eq!(13, i32.get_value().unwrap());
    }

    #[test]
    fn test_decrement() {
        let mut i32 = ValueI32::init_with(12);
        let result = i32.decrement();
        if let Value::I32(v) = &result.unwrap() {
            assert_eq!(12, v.get_value().unwrap());
        }

        assert_eq!(11, i32.get_value().unwrap());
    }

    #[test]
    fn test_not() {
        let mut i32 = ValueI32::init_empty();
        let mut result = i32.not();
        if let Value::Bool(bool_result) = &result {
            assert_eq!(true, bool_result.get_value().unwrap());
        } else { panic!(); }

        i32 = ValueI32::init_with(0);
        result = i32.not();
        if let Value::Bool(bool_result) = &result {
            assert_eq!(true, bool_result.get_value().unwrap());
        } else { panic!(); }

        i32 = ValueI32::init_with(1);
        result = i32.not();
        if let Value::Bool(bool_result) = &result {
