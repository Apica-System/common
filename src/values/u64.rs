...
impl ValueU64 {
    ...
    pub fn convert(&self, to: ApicaTypeBytecode) -> Option<Value> {
        if let Some(value) = &self.value {
            match to {
                ...
                ApicaTypeBytecode::Type => Some(Value::Type(ValueType::init_with(ApicaTypeBytecode::U64, None))),
    ...
    pub fn auto_convert(&self, to: ApicaTypeBytecode) -> Option<Value> {
        if let Some(value) = &self.value {
            match to {
                ApicaTypeBytecode::Any => Some(Value::U64(ValueU64::init_with(*value))),
                ...
                ApicaTypeBytecode::U64 => Some(Value::U64(ValueU64::init_with(*value))),
            ...
            match to {
                ApicaTypeBytecode::Any => Some(Value::U64(ValueU64::init_empty())),
...
mod tests {
    ...
    #[test]
    fn test_empty() {
        let mut u64 = ValueU64::init_empty();
        assert!(u64.is_null());

        u64 = ValueU64::init_with(12);
        assert!(!u64.is_null());
    }

    #[test]
    fn test_type_repr() {
        let u64 = ValueU64::init_empty();
        assert_eq!("u64", u64.get_type_representation());
    }

    #[test]
    fn test_get_value() {
        let mut u64 = ValueU64::init_empty();
        assert!(u64.get_value().is_none());

        u64 = ValueU64::init_with(12);
        assert_eq!(12, u64.get_value().unwrap());
    }

    #[test]
    fn test_add() {
        let u64 = ValueU64::init_with(12);

        let i16 = Value::I16(ValueI16::init_with(-12));
        let mut result = u64.add(&i16);
        if let Value::I64(i16_result) = &result.unwrap() {
            assert_eq!(0, i16_result.get_value().unwrap());
        } else { panic!(); }

        let other_u64 = Value::U64(ValueU64::init_with(10));
        result = u64.add(&other_u64);
        ...
        result = u64.add(&char);
        if let Value::U64(char_result) = &result.unwrap() {
            assert_eq!(109, char_result.get_value().unwrap());
        } else { panic!(); }

        let string = Value::String(ValueString::init_with(String::new()));
        result = u64.add(&string);
        assert!(result.is_none());
    }

    #[test]
    fn test_increment() {
        let mut u64 = ValueU64::init_with(12);
        let result = u64.increment();
        if let Value::U64(v) = &result.unwrap() {
            assert_eq!(12, v.get_value().unwrap());
        } else { panic!(); }

        assert_eq!(13, u64.get_value().unwrap());
    }

    #[test]
    fn test_decrement() {
        let mut u64 = ValueU64::init_with(12);
        let result = u64.decrement();
        if let Value::U64(v) = &result.unwrap() {
            assert_eq!(12, v.get_value().unwrap());
        }

        assert_eq!(11, u64.get_value().unwrap());
    }

    #[test]
    fn test_not() {
        let mut u64 = ValueU64::init_empty();
        let mut result = u64.not();
        if let Value::Bool(bool_result) = &result {
            assert_eq!(true, bool_result.get_value().unwrap());
        } else { panic!(); }

        u64 = ValueU64::init_with(0);
        result = u64.not();
        if let Value::Bool(bool_result) = &result {
            assert_eq!(true, bool_result.get_value().unwrap());
        } else { panic!(); }

        u64 = ValueU64::init_with(1);
        result = u64.not();
        if let Value::Bool(bool_result) = &result {
