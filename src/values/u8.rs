...
impl ValueU8 {
    ...
    pub fn add(&self, other: &Value) -> Option<Value> {
        match other {
            Value::I8(i8) => Some(Value::I8(ValueI8::init_with(self.value.unwrap() as i8 + i8.get_value().unwrap()))),
            ...
            Value::U8(u8) => Some(Value::U8(ValueU8::init_with(self.value.unwrap() + u8.get_value().unwrap()))),
            Value::U16(u16) => Some(Value::U16(ValueU16::init_with(self.value.unwrap() as u16 + u16.get_value().unwrap()))),
            ...
            Value::Bool(bool) => Some(Value::U8(ValueU8::init_with(self.value.unwrap() + if bool.get_value().unwrap() { 1 } else { 0 }))),
    ...
    pub fn convert(&self, to: ApicaTypeBytecode) -> Option<Value> {
        if let Some(value) = &self.value {
            match to {
                ...
                ApicaTypeBytecode::Type => Some(Value::Type(ValueType::init_with(ApicaTypeBytecode::U8, None))),
    ...
    pub fn auto_convert(&self, to: ApicaTypeBytecode) -> Option<Value> {
        if let Some(value) = &self.value {
            match to {
                ApicaTypeBytecode::Any => Some(Value::U8(ValueU8::init_with(*value))),
                ...
                ApicaTypeBytecode::U8 => Some(Value::U8(ValueU8::init_with(*value))),
                ...
                ApicaTypeBytecode::Char => Some(Value::Char(ValueChar::init_with(*value as char))),

                _ => None,
            }
        } else {
            match to {
                ApicaTypeBytecode::Any => Some(Value::U8(ValueU8::init_empty())),
...
mod tests {
    ...
    #[test]
    fn test_empty() {
        let mut u8 = ValueU8::init_empty();
        assert!(u8.is_null());

        u8 = ValueU8::init_with(12);
        assert!(!u8.is_null());
    }

    #[test]
    fn test_type_repr() {
        let u8 = ValueU8::init_empty();
        assert_eq!("u8", u8.get_type_representation());
    }

    #[test]
    fn test_get_value() {
        let mut u8 = ValueU8::init_empty();
        assert!(u8.get_value().is_none());

        u8 = ValueU8::init_with(12);
        assert_eq!(12, u8.get_value().unwrap());
    }

    #[test]
    fn test_add() {
        let u8 = ValueU8::init_with(12);

        let i16 = Value::I16(ValueI16::init_with(-12));
        let mut result = u8.add(&i16);
        ...
        result = u8.add(&string);
        assert!(result.is_none());
    }

    #[test]
    fn test_increment() {
        let mut u8 = ValueU8::init_with(12);
        let result = u8.increment();
        if let Value::U8(v) = &result.unwrap() {
            assert_eq!(12, v.get_value().unwrap());
        } else { panic!(); }

        assert_eq!(13, u8.get_value().unwrap());
    }

    #[test]
    fn test_decrement() {
        let mut u8 = ValueU8::init_with(12);
        let result = u8.decrement();
        if let Value::U8(v) = &result.unwrap() {
            assert_eq!(12, v.get_value().unwrap());
        }

        assert_eq!(11, u8.get_value().unwrap());
    }

    #[test]
    fn test_not() {
        let mut u8 = ValueU8::init_empty();
        let mut result = u8.not();
        if let Value::Bool(bool_result) = &result {
            assert_eq!(true, bool_result.get_value().unwrap());
        } else { panic!(); }

        u8 = ValueU8::init_with(0);
        result = u8.not();
        if let Value::Bool(bool_result) = &result {
            assert_eq!(true, bool_result.get_value().unwrap());
        } else { panic!(); }

        u8 = ValueU8::init_with(1);
        result = u8.not();
        if let Value::Bool(bool_result) = &result {
